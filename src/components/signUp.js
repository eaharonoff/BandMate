...
class SignUp extends Component {
  ...
  otherFunc(props){
    ...
    axios({
      method: 'get',
      url: `https://maps.googleapis.com/maps/api/geocode/json?address=${city}&key=AIzaSyA4X16Aq4qYw7WrqcvZGzdKgeeL26E5irc`
    }).then((response) => {
  ...
  render() {
    const {fields: {name, city, email, password}, handleSubmit} = this.props;
    return (
      <div className='container'>
        <div id='errors'></div>
          <form className='col-centered' onSubmit={handleSubmit(this.otherFunc.bind(this))}>
              ...
              <input type='text' className='form-control' placeholder='City/State' required {...city}/><br />
              <label >Email: </label>
